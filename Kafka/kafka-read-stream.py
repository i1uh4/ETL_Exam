...
OUTPUT_PATH     = 's3a://dataproc-bucket-etl-3-task/kafka-read-stream-output'
...
def main():
    ...
    parsed = raw.select(F.from_json(F.col("value"), schema).alias("j")).select("j.*")
    ...
    flat.coalesce(1).write.mode("overwrite") \
        .option("header", "true") \
        .csv(OUTPUT_PATH)
